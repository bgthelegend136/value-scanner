...
$TaskName = 'Bet-Live-Shadow'
$Runner = Join-Path $PSScriptRoot 'run-live-shadow-probe.ps1'
...
# A live-shadow run lasts two hours; the repetition cadence starts the next
# run after the previous one finishes. MultipleInstances avoids overlapping
# WebSocket probes if a run is still shutting down.
...
Register-ScheduledTask `
  ...
  -Settings $Settings `
  -Principal $Principal `
  -Description 'Measurement-only live shadow WebSocket probe. Writes local CSVs; no Telegram and no betting.' `
  -Force | Out-Null
